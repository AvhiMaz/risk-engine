...
#define PROGRAM_ID_STR "44iCEak3d798Gau411YoDisRFNMyZuHM941oZWYfx95U"
...
static int is_on_ed25519_curve(const uint8_t bytes[32]) {
    ...
    y_le[31] &= 0x7F;
    for (int i = 0; i < 32; i++) y_be[i] = y_le[31 - i];
    BN_bin2bn(y_be, 32, y);
    ...
    uint8_t d_be[32];
    for (int i = 0; i < 32; i++) d_be[i] = d_le[31 - i];
    BN_bin2bn(d_be, 32, d);
...
int main(void) {
    ...
    uint64_t nonce        = 0;
    uint64_t entry_price  = 681245; /* $68.1245 scaled to 4 decimals */
    uint64_t size         = 10;
    uint64_t margin       = 100;
    uint8_t  side         = 0; /* LONG */
    ...
    tx[off++] = 4;
    memcpy(tx + off, kp.pubkey,       32); off += 32; /* [0] trader */
    memcpy(tx + off, position_pubkey, 32); off += 32; /* [1] position */
    memcpy(tx + off, SYSTEM_PROGRAM,  32); off += 32; /* [2] system_program */
    memcpy(tx + off, program_id,      32); off += 32; /* [3] program */
    ...
    memcpy(tx + off, &entry_price,  8); off += 8;
    memcpy(tx + off, &size,         8); off += 8;
    memcpy(tx + off, &margin,       8); off += 8;
    tx[off++] = side;
    memcpy(tx + off, &market_index, 4); off += 4;
    memcpy(tx + off, &nonce,        8); off += 8;
    tx[off++] = bump;
